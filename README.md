...
<sub>If you get a `Could not write domain` error, this is usually solved by giving Terminal [Full Disk Access](https://github.com/mathiasbynens/dotfiles/issues/1027), but you can avoid doing that by using following trick:</sub><br><sub>In Terminal first run the command:</sub><br><sub>`ls $HOME/Library/Containers/com.apple.Safari/Data/Library/Preferences/com.apple.Safari.plist`</sub><br><sub>As you can see, it doesn't work, but the following does:</sub><br><sub>`open $HOME/Library/Containers/com.apple.Safari/Data/Library/Preferences/`</sub><br><sub>Then in the Finder window that opens, left click the file 'com.apple.Safari.plist' and copy it with Cmd+C, and then in Terminal again:</sub><br><sub>`ls` followed by space, then paste the copied file with Cmd+V and then press Enter (doesn't have to be `ls` can be any command and also works with other files that requires Full Disk Access permission by Terminal)</sub><br><sub>Finally run the commands from the previous step again</sub>
